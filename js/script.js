...
$(function () {
  ...
  $(window).on("load resize", function () {
    ...
    if (window.matchMedia(mediaQuery.large).matches) {
      $(".article__note").each(function () {
        var marginTopNext = parseInt($(this).next().css("marginTop"), 10);
        var marginBottomPrev = parseInt($(this).prev().css("marginBottom"), 10);
        var marginTop = Math.abs(marginTopNext - marginBottomPrev);
        $(this).css({
  ...
  $(window).on("load resize", function () {
    var image = "".concat($(".article-hero__img").attr("src"));
    var overlay = "linear-gradient(rgba(138, 70, 198, 0.80) 0%, rgba(235, 87, 87, 0.5) 100%), linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5))";

    if (window.matchMedia(mediaQuery.large).matches) {
      $(".article-hero").css({
        'backgroundImage': "".concat(overlay, ", url(\"").concat(image, "\")")
      });
      $(".article-hero--main .container").append($(".top-posts"));
    } else if (window.matchMedia(mediaQuery.medium).matches) {
      $(".article-hero").css({
        'backgroundImage': "".concat(overlay, ", url(\"").concat(image, "\")")
      });
  ...
  $(".modal-trigger").on("click", function () {
    ...
    if (window.matchMedia(mediaQuery.smallOnly).matches) {
      ...
      $("body").css("top", "".concat(-scrollTop, "px"));
    }
  });
  ...
  (function () {
    var resizeIndicator = function resizeIndicator(tab) {
      var tabPosition = tab.position();
      var tabPositionLeft = tabPosition.left + parseInt(tab.css("marginLeft"), 10);
      var tabWidth = tab.innerWidth();
      tab.siblings(".tabs__indicator").attr("style", "left:".concat(tabPositionLeft, "px; width:").concat(tabWidth, "px"));
    };

    var setTabActive = function setTabActive(tab) {
      var contentID = tab.data("content");
      var content = tab.closest(".tabs").find(contentID);
      tab.siblings(".tabs__item").removeClass("active").end().addClass("active");
      content.siblings(".tabs__content-item").removeClass("active").end().addClass("active");
    };
  ...
  $(window).on("load resize", function () {
    ...
    if (window.matchMedia(mediaQuery.medium).matches) {
      $(".how-it-works__scheme-wrapper").niceScroll(".how-it-works__scheme", niceScrollConf);
    } else {
  ...
  $(".program__lesson-top").on("click", function () {
    ...
    var video = $(this).closest(".program__lesson").find("iframe");
    video.attr("src", video.attr("src"));
  });
  $(window).on("load resize", function () {
    if (!window.matchMedia(mediaQuery.medium).matches) {
      $(".reviews__avatar").each(function (i, avatar) {
        if (i > 7) {
  ...
  $(".you-can__list .you-can__item:first-child").addClass("active").next(".you-can__descr").slideDown("fast");
  ...
  (function () {
    ...
    $(".form__step [name=\"next\"]").on("click", function () {
      ...
      var steps = form.find(".form__step");
      var indicators = form.find(".form__steps-indicator-item");
      var currentStep = form.find(".form__step:visible:first");
      var stepIndex = currentStep.index() + 1;
      var nextStep = steps.eq(stepIndex);
      currentStep.hide();
